...
function App() {
	...
	return (
		<div className="min-h-screen">
			<div className="text-center bg-stone-800 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${song.coverURL})` }}>
				<aside className={`${(isHidden? "translate-x-[100%]" : "translate-x-0")} lg:translate-x-0 transition delay-150 duration-300 ease-linear z-40 max-w-[100%] lg:max-w-[30%] lg:right-0 min-h-screen absolute lg:relative flex-col lg:float-right items-center justify-start sm:min-w-[20%] bg-slate-700/30 h-[50%] pt-4 space-y-6 backdrop-blur-lg lg:flex`}>
					{songs.map((song) => (
						<Playlist song={song} />
					))}
				</aside>
				<header className="h-screen flex flex-col items-center justify-center text-base text-white backdrop-blur-lg backdrop-brightness-50 relative">
